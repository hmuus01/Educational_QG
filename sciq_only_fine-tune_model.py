...
sciq_train_df = pd.read_csv('data/sciq/train.csv')
sciq_test_df = pd.read_csv('data/sciq/test.csv')
print(sciq_train_df.shape)

sciq_train_df.head()

sciq_dev_df = pd.read_csv('data/sciq/validation.csv')
print(sciq_dev_df.shape)
...
test_df = sciq_test_df
train_df = sciq_train_df
...
class QGDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        source_encoding = tokenizer(
            '{} {} {}'.format(answer, SEP_TOKEN, data_row['support']),
...
# tokenizer = AutoTokenizer.from_pretrained('./t_from_checkpoint_t5_pre') # compsci
...
checkpoint_callback = ModelCheckpoint(
    dirpath='checkpoints',
    filename='sci_small-fine_tuned_on_sciq_ft-test-best-checkpoint',
    save_top_k=-1,
    verbose=True,
    monitor='val_loss',
    mode='min'
